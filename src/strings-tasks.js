...
function isPalindrome(str) {
  const filteredStr = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
...
function encodeToRot13(str) {
  return str
    ...
    .map((char) => {
      ...
      const isOverflow = char.toUpperCase().charCodeAt(0) > 77; // M = 77
      if (
        (charCode >= 65 && charCode <= 90) ||
        (charCode >= 97 && charCode <= 122)
      ) {
        return String.fromCharCode(isOverflow ? charCode - 13 : charCode + 13);
      }
      return char;
    })
    .join('');
...
function getCardId(value) {
  ...
  return (
    cards.indexOf(value.slice(0, -1)) + suits.indexOf(value.slice(-1)) * 13
  );
}
